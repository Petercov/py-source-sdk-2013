...
public:
	...
	inline bool DeleteTextureByID(int id) { return surface()->DeleteTextureByID(id); }
	...
	inline void SetCursorAlwaysVisible( bool visible ) { surface()->SetCursorAlwaysVisible(visible); }
	...
	inline bool AddCustomFontFile(const char *fontName, const char *fontFileName) { return surface()->AddCustomFontFile(fontFileName, fontFileName); }

	// returns the details about the font
	inline int GetFontTall(HFont font) { return surface()->GetFontTall(font); }
	inline int GetFontTallRequested(HFont font) { return surface()->GetFontTallRequested(font); }
	...
	inline void PrecacheFontCharacters(HFont font, const wchar_t *pCharacters) { surface()->PrecacheFontCharacters(font, pCharacters); }
	...
	inline const char *GetFontFamilyName( HFont font ) { return surface()->GetFontFamilyName( font ); }
	inline void GetKernedCharWidth( HFont font, wchar_t ch, wchar_t chBefore, wchar_t chAfter, float &wide, float &abcA ) { surface()->GetKernedCharWidth( font, ch, chBefore, chAfter, wide, abcA ); }
	...
	inline void DrawUpdateRegionTextureRGBA( int nTextureID, int x, int y, const unsigned char *pchData, int wide, int tall, ImageFormat imageFormat ) 
	{ 
		surface()->DrawUpdateRegionTextureRGBA( nTextureID, x, y, pchData, wide, tall, imageFormat ); 
	}

	//inline bool BHTMLWindowNeedsPaint(IHTML *htmlwin) { surface()->BHTMLWindowNeedsPaint( htmlwin ); }

	inline const char *GetWebkitHTMLUserAgentString() { return surface()->GetWebkitHTMLUserAgentString(); }
